...
DriverEntry(PDRIVER_OBJECT DriverObject, PUNICODE_STRING) {
	...
	bool processCallbacks = false, threadCallbacks = false, imageCallbacks = false;
			...
			if (NT_SUCCESS(CmCallbackGetKeyObjectIDEx(&g_RegCookie, args->Object,
				...
				if (wcsncmp(name->Buffer, machine, ARRAYSIZE(machine) - 1) == 0) {
					...
					if (info) {
						...
						wcsncpy_s((PWSTR)((PUCHAR)&data + offset), keyNameLen / sizeof(WCHAR), name->Buffer, name->Length / sizeof(WCHAR));
						offset += keyNameLen;
						data.ValueNameOffset = offset;
						wcsncpy_s((PWSTR)((PUCHAR)&data + offset), valueNameLen / sizeof(WCHAR), preInfo->ValueName->Buffer, preInfo->ValueName->Length / sizeof(WCHAR));
						offset += valueNameLen;
						...
						KdPrint((DRIVER_PREFIX "Failed to allocate memory for registry set value notification\n"));
					}
				}
